...
class TanhRBM(RBM):
    ...
    def _initialize_vars(self):
        hidden_p = tf.math.tanh(tf.matmul(self.x, self.w) + self.hidden_bias)
        hidden_p = sample_gaussian(hidden_p, (hidden_p+1)*(hidden_p-1))
        ...
        hidden_recon_p = tf.nn.relu(tf.matmul(visible_recon_p, self.w) + self.hidden_bias)
        hidden_recon_p = sample_gaussian(hidden_recon_p, (hidden_recon_p+1)*(hidden_recon_p-1))
    ...
    def _compute_hidden(self):
        self.compute_hidden = tf.math.tanh(tf.matmul(self.x, self.w) + self.hidden_bias)
