...
class GBRBM(RBM):
    ...
    def PCD(self):
        hidden_p = tf.nn.sigmoid(tf.matmul(self.chain, self.w) + self.hidden_bias)
        self.chain = tf.matmul(sample_bernoulli(hidden_p), tf.transpose(self.w)) + self.visible_bias

        if self.sample_visible:
            self.chain = sample_gaussian(self.chain, self.sigma)

        return self.chain, hidden_p
    ...
    def get_energy(self, x=None, h=None):
        ...
        self.sigma = 1.0
        test = 1#(tf.reduce_sum(tf.square(x - self.visible_bias) / (2 * np.square(self.sigma)), 1))
        test2 = 1#tf.reduce_sum(self.hidden_bias * h, 1)
        test3 = 1#tf.linalg.tensor_diag_part(tf.matmul(tf.matmul(x / self.sigma, self.w), tf.transpose(h)))
